...
const text = [
    ...
    `Sometimes I laugh even when there is no one around thinking about all the funny things that you do just to make me happy.`,
...
const valEvent = [
    `make a special meal together:  going to the grocery store, preparing, cooking and serving the meal to each other can be incredibly meaningful.`,
    `write love notes to each other: taking time to put down in words why we love each other can be incredibly valuable.`,
    `have a photoshoot together: documenting the two of us would help make special memories.`,
...
submit.addEventListener("click", function(e){
    ...
    if ((ageValue > 15 && ageValue < 60)|| (!personNameValue ==="") ) {
        alert("Please Take Time To Read Your Lover's Text");
...
const randomMusic = Math.floor(Math.random() * music.length);
function changeMusicSrc () {
    audio.setAttribute("src", `${music[randomMusic]}`);
}